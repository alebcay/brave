...
websocket._handleDelete = function(item) {
  const handler = websocket._getHandlerForBlockType(item.block_type);
  if (handler) {
...
websocket._setCpuPercent = (num) => {
  const cpuString = num.toFixed(1);
  const cpuLabel = `CPU: ${cpuString.padStart(5 - cpuString.length, '0')}%`;
  $('#cpu-stats')
    .empty()
    .text( cpuLabel );
};
